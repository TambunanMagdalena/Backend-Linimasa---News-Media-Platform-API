...
module.exports = {
  // REGISTER TANPA UPLOAD GAMBAR (JSON)
  ...
  // REGISTER DENGAN UPLOAD GAMBAR (FORM-DATA)
  registerWithImage: async (request, response) => {
    try {
      ...
      console.log("Register With Image Error:", error);
      miscHelper.customErrorResponse(response, 404, "Cannot register user!");
    }
  },
  ...
  updateUserWithImage: async (request, response) => {
    try {
      const user_id = request.params.user_id;
      ...
      console.log("Update User With Image Error:", error);
      miscHelper.customErrorResponse(response, 404, "Cannot update user!");
    }
  },
    ...
    try {
      ...
      if (dataUser !== undefined) {
        ...
        if (hashPassword.passwordHash === dataUser.user_password) {
          ...
          const refresh_token = JWT.sign(user, JWT_Refresh, {
            expiresIn: "7d",
          });
      ...
      miscHelper.customErrorResponse(response, 404, "Cannot login!");
    }
  },
  ...
  token: async (request, response) => {
    try {
      ...
      if (
        setData.refresh_token == tokenList[setData.user_email]?.refresh_token
      ) {
        const user = {
          user_email: tokenList[setData.user_email].user_email,
          id: tokenList[setData.user_email].id,
        };
        const token = JWT.sign(user, JWT_Key, { expiresIn: "1h" });
      ...
      miscHelper.customErrorResponse(response, 404, "Cannot refresh token!");
    }
  },
